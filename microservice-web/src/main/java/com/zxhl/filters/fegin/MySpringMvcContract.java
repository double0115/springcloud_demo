 ...
 */
public class MySpringMvcContract extends Contract.BaseContract
    ...
    protected boolean processAnnotationsOnParameter(MethodMetadata data,
        ...
        for (Annotation parameterAnnotation : annotations) {
            ...
            if (annotationType == RequestParam.class) {
                RequestParam paramAnnotation = (RequestParam) parameterAnnotation;
                ...
                //如果使用pathvalible   value的命名需要和接口的参数名保持一致，否则会出现urlencode问题
                PathVariable paramAnnotation = (PathVariable) parameterAnnotation;
                String name = paramAnnotation.value();
                if(!StringUtil.isNullOrEmpty(name)) {
                    nameParam(data, name, paramIndex);
                    //参数统一不编码
